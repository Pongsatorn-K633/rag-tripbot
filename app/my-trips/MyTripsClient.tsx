...
/**
 * The /my-trips UI. Its trips arrive already-loaded from the server page
 * (`initialTrips`) — there is no fetch-on-mount and therefore no loading state:
 * the very first paint is either the real cards or the empty state. Everything
 * after mount (delete, LINE code, the view modal) still mutates this local copy.
 */
export default function MyTripsClient({
  ...
  // useSession is still here so a profile edit (session.update()) reflows the
  // owner line without a reload — but the SERVER's answer is what the first
  // paint uses, because useSession reports "unauthenticated" for a tick while
  // it hydrates, which would flash the sign-in CTA at a signed-in user.
  ...
  /**
   * "16 ต.ค. – 24 ต.ค. 2569" — the year rides on the END date only, since a
   * trip almost always sits inside one year and repeating it reads as noise.
   * A range that DOES cross new year ("28 ธ.ค. 2569 – 3 ม.ค. 2570") gets both,
   * which is exactly when the reader needs them.
   *
   * th-TH prints the Buddhist year (2569), matching every other Thai date in
   * the app.
   */
  function formatDateRange(startDateStr: string, days: number): string {
    ...
    return `${fmt(start, crossesYear)} - ${fmt(end, true)}`
  }
                  ...
                  return (
                    ...
                    >
                      <MyTripCardCompact
                        ...
                        ownerName={user?.name ?? user?.email ?? 'คุณ'}
